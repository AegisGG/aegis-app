const guideSteps = [
  'Connect wallet',
  'Press Enable Skating to allow Staking of AEGIS tokens',
  'Approve the Allowance Pop-up',
  'Input desired amount of AEGIS tokens or press the Max button to wager',
  'There is a 10% fee when participating on Wagers',
  'Press Stake',
  'User can unstake anytime before matches start but will only get 90% of original staked amount due to fees',
  'Press Claim Prize to receive payout.'
];

export default guideSteps;
